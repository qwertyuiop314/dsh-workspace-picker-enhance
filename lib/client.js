window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const { Modal, Button, Tooltip } = primitives;

		/**
		 * DSH 0.1.7 把图标导出从像素尺寸后缀（…14/…16/…20）改成了描边粗细后缀
		 * （…Regular/…Medium）。这里两代名字都探一遍，让同一份 bundle 在 0.1.5
		 * 与 0.1.7+ 上都能拿到组件；两代都没有时仍是 undefined。
		 */
		const icon = (legacy, modern) => primitives[modern] ?? primitives[legacy];
		const IconChevronRightOutline14 = icon('IconChevronRightOutline14', 'IconChevronRightOutlineRegular');
		const IconChevronUpOutline14 = icon('IconChevronUpOutline14', 'IconChevronUpOutlineRegular');
		const IconFolderOpen16 = icon('IconFolderOpen16', 'IconFolderOpenRegular');
		const IconWarningOutline16 = icon('IconWarningOutline16', 'IconWarningOutlineRegular');
		const IconEditOutline16 = icon('IconEditOutline16', 'IconEditOutlineRegular');
		const IconRefreshOutline16 = icon('IconRefreshOutline16', 'IconRefreshOutlineRegular');
...
body[data-ds-dark-theme] .wpe-row-icon-warn { color: var(--dsw-static-amber-400); }
body[data-ds-dark-theme] .wpe-row-system:not(.wpe-row-no-permission) .wpe-row-name { color: var(--dsw-static-amber-400); }
body[data-ds-dark-theme] .wpe-toast { background: var(--dsw-alias-bg-layer-3); }
`;
