...
`;
		...
		// ---------- API ----------
		const api = {
			roots: () => fetch('/plugins/workspace-picker-enhance/roots').then((r) => r.json()),
			list: (path) => fetch('/plugins/workspace-picker-enhance/list?path=' + encodeURIComponent(path)).then((r) => r.json()),
		};
		...
		function EnhancedDirectoryFlow(props) {
			...
			const renderRow = (entry, index) => {
				...
				const row = h('button', {
					...
					className: 'wpe-row' + (blocked ? ' wpe-row-no-permission' : '') + (entry.system ? ' wpe-row-system' : ''),
